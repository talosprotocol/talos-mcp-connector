...
Enforces Phase 9.2 Tool Policies (Read/Write separation) and Phase 9.3 Idempotency.
"""
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # In dev mode, we might want to continue with empty config
        if os.getenv("TALOS_ENV") != "dev":
             raise

    # Initialize Policy Engine
    registry_path = os.getenv("TALOS_MCP_TOOL_REGISTRY_PATH", "tool_registry.json")
...
async def call_tool(
    ...
    try:
        ...
        # Check Idempotency (Presence check for write tools)
    ...
    # 3. Execute Tool (with Durable Idempotency)
    server_conf = state.config.mcpServers[server_id]
